$(function(){
    ...
    $('#dg').datagrid({
        ...
        columns:[[
            ...
            {field:'name',title:'房屋类型',width:100},
    ...
    $('#addDialogForm').form('submit', {
        ...
        success:function(data){//返回的数据时字符串
            ...
            if (data.result==1) {
                ...
                $.messager.alert('提示','添加房屋信息成功!','info');


            }else {
                $.messager.alert('提示','添加房屋信息失败!','info');
...
function delete1(obj) {
    ...
    $.messager.confirm('提示框', '确认删除数据吗?', function(r){
        if (r){
            $.post("delOneType",{"id":obj},function (data) {
                if (data.result==1) {
...
function DeleteMoreType() {
    ...
    if (selectRows.length>0){
        $.messager.confirm('提示框', '确认删除吗?', function(r){
            ...
            $.post("delMoreType",{"ids":vel},function (data) {
                if (data.result>0) {
